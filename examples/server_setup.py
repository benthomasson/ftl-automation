...
def main():
    ...
    with ftl_automation.automation(
        ...
        server_name = user_input_tool(
            question="Enter server name:",
            default="web-server-01"
        )
        
        domain_name = user_input_tool(
            question="Enter domain name (e.g. example.com):",
            default="localhost"
        )
        ...
        try:
            ...
            update_result = ftl.run_module("dnf", 
                name="*", 
                state="latest",
                update_cache=True
            )
            ...
            try:
                ...
                update_result = ftl.run_module("apt",
                    upgrade="yes",
                    update_cache=True
                )
                pkg_manager = "apt"
        ...
        install_result = ftl.run_module("package",
            name="nginx",
            state="present"
        )
        ...
        service_result = ftl.run_module("service",
            name="nginx",
            state="started",
            enabled=True
        )
        ...
        try:
            # Try firewalld first
            firewall_http = ftl.run_module("firewalld",
                service="http",
                permanent=True,
                state="enabled"
            )
            
            firewall_https = ftl.run_module("firewalld", 
                service="https",
                permanent=True,
                state="enabled"
            )
    ...
    add_header X-XSS-Protection "1; mode=block";
        ...
        if ssl_domain and ssl_email and domain_name != "localhost":
            ...
            try:
                ...
                # Get SSL certificate
                cert_result = ftl.run_module("command",
        ...
        if status_result and "active" in str(status_result):
            ...
            complete(f"Web server {server_name} deployed successfully with domain {domain_name}")
        else:
