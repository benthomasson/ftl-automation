...
def deploy_web_server(inventory, modules, console, server_name, **kwargs):
    ...
    steps = [
        ...
        (
            ...
            "file",
            {"path": f"/var/www/{server_name}", "state": "directory"},
        ),
        (
            "Setting permissions",
            "file",
            {"path": f"/var/www/{server_name}", "mode": "0755"},
        ),
    ]
...
def backup_and_update(inventory, modules, console, service_name, **kwargs):
    ...
    try:
        ...
        backup_result = ftl_automation.run_module(
            ...
            module_name="command",
            module_args={
                "cmd": f"systemctl stop {service_name} && tar -czf /tmp/{service_name}_backup.tar.gz /etc/{service_name}"
            },
        )
        ...
        try:
            ftl_automation.run_module(
                ...
                module_name="command",
                module_args={
                    "cmd": f"tar -xzf /tmp/{service_name}_backup.tar.gz -C / && systemctl start {service_name}"
                },
            )
...
def main():
    ...
    try:
        ...
        with ftl_automation.automation(
            inventory=inventory_file, modules=["modules"], tools_files=[__file__]
        ) as ftl:
            ...
            ftl.print(
                f"[green]Deployment result: {len(result)} steps completed[/green]"
            )
        ...
        with ftl_automation.automation(
            ...
            try:
                ...
                result = ftl.execute_tool(
                    "backup_and_update", service_name="nonexistent-service"
                )
        ...
        # Production context
        with ftl_automation.automation(
            inventory=inventory_file, extra_vars={"env": "prod"}
        ) as prod_ftl:
        ...
        # Development context
        with ftl_automation.automation(
            inventory=inventory_file, extra_vars={"env": "dev"}
        ) as dev_ftl:
